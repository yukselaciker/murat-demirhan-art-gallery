...
function getCloudflareAccessUser(request) {
...
__name(getCloudflareAccessUser, "getCloudflareAccessUser");
function isAdminPath(path, env) {
  const adminPrefix = env.ADMIN_PATH_PREFIX || "/studio-md-2024";
  return path.startsWith(adminPrefix);
}
__name(isAdminPath, "isAdminPath");
function accessDeniedResponse(origin) {
  return new Response(
    JSON.stringify({
      error: "Access Denied",
      message: "This resource requires Cloudflare Access authentication"
    }),
    {
      status: 403,
      headers: {
...
__name(accessDeniedResponse, "accessDeniedResponse");
...
var index_default = {
  async fetch(request, env) {
    ...
    try {
      if (isAdminPath(path, env)) {
        const accessUser = getCloudflareAccessUser(request);
        const isLocalhost = origin?.includes("localhost") || origin?.includes("127.0.0.1");
        if (!isLocalhost && !accessUser) {
          return accessDeniedResponse(origin);
        }
        if (accessUser) {
          console.log(`Admin access by: ${accessUser} to ${path}`);
        }
      }
